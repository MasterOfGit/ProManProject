...
{
	"use strict";
	alert("Start ExecuteDeleteAction for id:" + id);
	...
	alert("Finish ExecuteDeleteAction for id:"+id);	
}
...
function updateBauteil(data,id) {
	...
	"use strict";
	alert("Starte Update Bauteil");
	ExecuteUpdateActionDbCreateActionDb("bauteil",data,id);
...
function saveMaschine(data) {
	...
	if(rawdata["maschinenID"]==0)
	{
		ExecuteCreateActionDb("maschine",data);			
	}
	else
	{
		ExecuteUpdateActionDb("maschine",data,rawdata["maschinenID"]);
	}
}
...
function updateMaschine(data) {
	...
	"use strict";
	ExecuteUpdateActionDb("maschine",data,data["maschinenID"]);
}
...
function editWartung(wartungsID) {
	...
	"use strict";
	OpenEditPage("Sonderaufgaben/adminCRWartungBear.php?q=0"+wartungsID,"Wartung editiert");
}
...
function saveWartung(data, zugriff){
	...
	if(zugriff == 1) 
	{
		ExecuteUpdateActionDb("wartung",data,data["wartungId"]);
	}
...
function editAudit(AuditID) {
	"use strict";
	OpenEditPage("Sonderaufgaben/adminCRAuditsBear.php?q=0"+AuditID,"Wartung editiert");
}
