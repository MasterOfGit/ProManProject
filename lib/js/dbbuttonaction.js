...
function editBauteil(bauteilID) {
	...
	//var url = "http://localhost:8080/lib/php/admin/Bauteil/adminCRBauteilBear.php?q=";
...
function saveBauteil(data) {
	...
	"use strict";
	var url_bauteil = "http://zoomnation.selfhost.eu:8080/ProManAPI/API/bauteil";
	...
	$("#systemstatus").html("Neuer Bauteil wurde bearbeitet  !");
		...
		var url_audit = "http://zoomnation.selfhost.eu/lib/php/admin/bauteile/saveBauteilJsonDatei.php";
	...
	xhttp.onreadystatechange = function () {
		...
		if (this.readyState === 4 && this.status === 200) {
			$("#systemstatus").html("Bauteile Json Datei wurde gespeichert  !");
...
function editWartung(wartungsID) {
	...
	var url = "http://zoomnation.selfhost.eu/lib/php/admin/Sonderaufgaben/adminCRWartungBear.php?q=";
	...
	xhttp.open("GET", url + wartungsID, true);
	xhttp.setRequestHeader("Content-type", "application/json");
	xhttp.send();

	$("#systemstatus").html("Wartung editiert");
}

function saveWartung(data, zugriff){
		
	// Save data in Datenbank
	///////////////////////////////
	//Insert new Audit in Datenbank 
	///////////////////////////////
	var url_audit = "http://zoomnation.selfhost.eu:8080/ProManAPI/API/Wartungen";
	
	if(zugriff == 0)
		
		alert("Save Wartung (Insert)");
	...
	xhttp.onreadystatechange = function () {
		...
		if (this.readyState === 4 && this.status === 200) {
			$("#systemstatus").html("Neue Wartung wurde hinzugefügt !");

		}
		if (this.readyState === 4 && this.status === 204) {
			alert("Server saveWartung schickt keine Anwort:");

		}
	};

	xhttp.open("POST", url_audit, true);
	xhttp.setRequestHeader("Content-type", "application/json");
	xhttp.send(data);
	...
	xhttp.onreadystatechange = function () {
		...
		if (this.readyState === 4 && this.status === 200) {
			$("#systemstatus").html("Bestehendes Wartung wurde geaendert !");
...
function editAudit(AuditID) {
	...
	var url = "http://zoomnation.selfhost.eu/lib/php/admin/Sonderaufgaben/adminCRAuditsBear.php?q=";
	...
	xhttp.onreadystatechange = function () {
		...
		if (this.readyState === 4 && this.status === 200) {
			$("#daten").html(xhttp.responseText);
			$("#systemstatus").html("Audit editiert");
		...
		if (this.readyState === 4 && this.status === 204) {
			alert("Server saveData schickt keine Anwort:");
			$("#systemstatus").html("Keine Daten vo9m Server");

		}
	};
	xhttp.open("GET", url + AuditID, true);
	xhttp.setRequestHeader("Content-type", "application/json");
	xhttp.send();
...
// Save data in Datenbank
	...
	//Insert new Audit in Datenbank 
	///////////////////////////////
	var url_audit = "http://zoomnation.selfhost.eu:8080/ProManAPI/API/Audits";
	
	if(zugriff == 0)
	{
	alert("Save Audit (Insert)");
		
	xhttp.onreadystatechange = function () {
		
		if (this.readyState === 4 && this.status === 200) {
			$("#systemstatus").html("Neue Audits wurde hinzugefügt !");
...
function deleteAudit(auditID) {
	...
	alert("Audits wurden gespeichert");
}
